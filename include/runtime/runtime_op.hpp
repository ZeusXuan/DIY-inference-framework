...
struct RuntimeOperator {
  ...
  std::vector<std::string> output_names; /// 输出节点名称(可能有多个)
  std::shared_ptr<RuntimeOperand> output_operands; /// 输出操作数(只有一个)
  std::vector<std::shared_ptr<RuntimeOperand>> input_operands_seq; /// 输入操作数(可能有多个)
  std::map<std::string, std::shared_ptr<RuntimeOperand>> input_operands; /// 输出节点名字与输入操作数的哈希表
   
  std::map<std::string, std::shared_ptr<RuntimeOperator>> output_operators; /// 输出节点的名字和输出节点对应
